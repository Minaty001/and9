...
from .models import Event, AnalyticsReport, TimeSeriesPoint
...
class ReportGenerator:
    ...
    Usage:
        gen = ReportGenerator(usage_tracker, performance_tracker, config)
    ...
    def generate_report(self, report_type: str = "daily") -> AnalyticsReport:
        ...
        Returns:
            AnalyticsReport with metrics, insights, and anomalies.
        """
        ...
        # Anomalies
        anomalies = self._detect_anomalies(events) if self.config.enable_anomaly_detection else []
    ...
    def _empty_report(self, report_type: str, start: datetime, end: datetime) -> AnalyticsReport:
        """Create an empty report when no events exist."""
        return AnalyticsReport(
            report_id=uuid.uuid4().hex[:12],
            report_type=report_type,
            period_start=start,
    ...
    def _compute_charts(self, events: List[Event], start: datetime, end: datetime) -> dict:
        """Compute chart data (events by day)."""
    ...
    def _compute_performance_summary(self, events: List[Event]) -> dict:
        """Compute performance summary from events.

        Calculates latency percentiles, error rate, and identifies
        the slowest endpoint and busiest hour.
        ...
        # Error rate: events with "error" in the event_type or action
        error_events = [
            e for e in events
            if "error" in e.event_type.lower() or "error" in e.action.lower()
        ]
        ...
        # Slowest endpoint (by avg duration per endpoint tag)
        endpoint_durations = {}
        for e in events:
        ...
        if endpoint_durations:
            slowest_endpoint = max(
                endpoint_durations,
                key=lambda ep: sum(endpoint_durations[ep]) / len(endpoint_durations[ep]),
            )
    ...
    def _detect_anomalies(self, events: List[Event]) -> list:
        ...
        Looks for sudden drops or spikes in event frequency.
        ...
        if max_count > 0 and min_count >= 0:
            # Detect spike: hour with > 3x average
        ...
        # Check daily distribution for drops
