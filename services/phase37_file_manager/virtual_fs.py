"""
Phase 37 — Virtual File System.

In-memory virtual filesystem with CRUD operations.
Supports import/export from real filesystem, archive import/export, and path validation.
"""
...
class VirtualFileSystem:
    ...
    def __init__(self, config: Optional[FileManagerConfig] = None):
        ...
        self._contents: Dict[str, Any] = {}  # path -> content
    ...
    def _setup_defaults(self):
        ...
        root = FileItem(
            id=root_id,
            name=os.path.basename(self.config.base_path) or "root",
            path=self.config.base_path,
            file_type="directory",
            size_bytes=0,
            mime_type="inode/directory",
        )
        self._files[self.config.base_path] = root

    def _normalize_path(self, path: str) -> str:
        """Normalize a file path."""
        path = path.replace("\\", "/")
        if not path.startswith(self.config.base_path):
            path = os.path.normpath(os.path.join(self.config.base_path, path.lstrip("/")))
        else:
    ...
    def validate_path(self, path: str, allow_absolute: bool = False) -> bool:
        ...
        # Check that path is within base directory (when base_path is set)
        base = os.path.normpath(self.config.base_path)
    ...
    def import_file(self, source_path: str, dest_path: str) -> Optional[FileItem]:
        ...
        Args:
            source_path: Absolute or relative path on the real filesystem.
            dest_path: Target path within the VFS.

        Returns:
            FileItem if imported successfully, None otherwise.
        """
    ...
    def export_file(self, vfs_path: str, dest_path: str) -> bool:
        ...
        Args:
            vfs_path: Path within the VFS.
            dest_path: Target path on the real filesystem.
    ...
    def import_archive(self, path: str, vfs_dir: str = "") -> int:
        """Import files from a zip or tar archive into the VFS.

        Args:
            path: Path to the archive file.
            vfs_dir: Optional subdirectory within VFS to place files.
    ...
    def _import_tar(self, path: str, vfs_dir: str) -> int:
        """Import files from a tar archive."""
        imported = 0
    ...
    def export_archive(self, path: str, vfs_paths: Optional[List[str]] = None) -> bool:
        ...
        Args:
            ...
            vfs_paths: List of VFS paths to export. If None, exports everything.
            ...
            with zipfile.ZipFile(path, "w", zipfile.ZIP_DEFLATED) as zf:
                for vp in paths_to_export:
                    ...
                    if item and item.file_type == "file":
                        ...
                        arcname = os.path.relpath(vp, self.config.base_path)
                        if isinstance(content, str):
    ...
    def _check_extension(self, path: str) -> bool:
        """Check if file extension is allowed/blocked."""
        _, ext = os.path.splitext(path)
        if ext.lower() in self.config.blocked_extensions:
            return False
        if self.config.allowed_extensions and ext.lower() not in self.config.allowed_extensions:
            return False
        return True
    ...
    def create_file(self, path: str, content: Any = "") -> Optional[FileItem]:
        ...
        Returns the FileItem or None on failure.
        """
    ...
    def delete(self, path: str) -> bool:
        ...
        if item.file_type == "directory":
            ...
            to_delete.sort(reverse=True)  # delete children first
            for p in to_delete:
    ...
    def copy(self, src: str, dest: str) -> bool:
        ...
        if src_item.file_type == "file":
            ...
            for item_path, item in items_to_copy:
                ...
                if item.file_type == "directory":
                    dir_id = uuid.uuid4().hex[:12]
                    new_item = item.copy(update={"id": dir_id, "path": new_path, "name": os.path.basename(new_path)})
    ...
    def list_directory(self, path: str) -> Optional[DirectoryEntry]:
        ...
        Returns DirectoryEntry or None if path not found.
        """
        path = self._normalize_path(path)
