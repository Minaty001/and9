...
class InputValidator:
    ...
    def validate(self, input_text: str) -> ValidationResult:
        ...
        if len(input_text) > self.config.max_input_length:
            errors.append(f"Input exceeds max length of {self.config.max_input_length}")
            risk_score = 1.0
            return ValidationResult(
                is_valid=False,
                errors=errors,
                sanitized_input=input_text[: self.config.max_input_length],
    ...
    def _check_prompt_injection(self, input_text: str):
        ...
        Returns:
            Tuple of (detected: bool, patterns_found: list).
        """
        detected = False
        patterns_found = []
        for pat in PROMPT_INJECTION_PATTERNS:
