...
from backend.cognition.neural.responses import get_response, INTENT_TO_ACTION, INTENT_TO_TYPE
from backend.cognition.neural.rag import RAGEngine, get_rag_response
...
class NeuralBridge:
    ...
    def _ensure_loaded(self):
        """Lazy-load the micro_brain on first call."""
        if self._loaded:
            return True

        try:
            # Add project root to path for micro_brain imports
            ...
            # Import and init MicroNeuralBrain
            from ai.micro_brain.main import MicroNeuralBrain

            self._brain = MicroNeuralBrain(verbose=False)
            if not self._brain.initialize():
                logger.error("NeuralBridge: MicroNeuralBrain init failed")
                return False

            self._reflex = self._brain.reflex
            self._loaded = True
            logger.info("NeuralBridge: loaded successfully")
            return True
        except Exception as e:
            logger.error("NeuralBridge: Failed to load micro_brain: %s", e)
            return False
    ...
    def _get_rag_context(self, query: str, intent: str):
        ...
        if self._rag is None:
            try:
                ...
                from backend.cognition.neural.rag import RAGContext
                return RAGContext()
        try:
