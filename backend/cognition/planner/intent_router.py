...
def _get_neural_brain():
    ...
    if _NEURAL_BRAIN is None:
        import sys
        import os
        mb_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", "ai/micro_brain"))
...
def detect_intent(query: str) -> Tuple[Optional[str], Optional[str], dict]:
    ...
    from backend.utils.timezone_utils import detect_city_time_query
...
def detect_intent_with_confidence(query: str) -> Tuple[Optional[str], Optional[str], dict, float]:
    ...
    from backend.cognition.planner.confidence_scorer import score_intent
...
def _extract_duration(query: str) -> Optional[int]:
    ...
    from backend.utils.time_parser import parse_duration
...
def _extract_label(query: str) -> Optional[str]:
    ...
    from backend.cognition.planner.entity_extractor import _extract_label
