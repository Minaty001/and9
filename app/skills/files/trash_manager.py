...
class TrashManager:
    ...
    Usage:
        mgr = TrashManager()
        mgr.move_to_trash("/path/to/file", item)
    ...
    def __init__(self, retention_days: int = 7):
        ...
        self._trash: Dict[str, Dict[str, Any]] = {}
    ...
    def restore(self, path: str) -> bool:
        candidates = [(tid, entry) for tid, entry in self._trash.items() if entry["original_path"] == path]
        if not candidates:
            return False
        ...
        return [
            {
                ...
                "expires_at": (entry["trashed_at"] + timedelta(days=self._retention_days)).isoformat(),
            }
    ...
    def cleanup_expired(self) -> int:
        ...
        for tid, entry in self._trash.items():
            ...
            if age.days >= self._retention_days:
