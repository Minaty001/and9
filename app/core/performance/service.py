...
Wrapper around all performance optimization components.
"""
...
class PerformanceConfig:
    """Configuration for PerformanceOptimizerService."""
    ...
    def __init__(self,
        ...
        self.l1_cache_ttl_seconds = l1_cache_ttl_seconds
        self.l2_cache_ttl_seconds = l2_cache_ttl_seconds
...
class PerformanceOptimizerService:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            if self.config.enable_l1_cache:
                self.l1 = L1Cache(
                    capacity=self.config.l1_cache_size,
                    ttl_seconds=self.config.l1_cache_ttl_seconds,
                    warmup_strategy=self.config.warmup_strategy,
                )
            if self.config.enable_l2_cache:
                self.l2 = L2Cache(
                    capacity=self.config.l2_cache_size,
                    ttl_seconds=self.config.l2_cache_ttl_seconds,
                    warmup_strategy=self.config.warmup_strategy,
                )
            ...
            self.profiler = BottleneckProfiler()
    ...
    def _check_init(self):
        if not self._initialized:
            raise RuntimeError("PerformanceOptimizerService not initialized")
    ...
    async def get_detailed_stats(self) -> Dict[str, Any]:
        ...
        stats = {}
        if self.l1:
            stats["l1_cache"] = vars(self.l1.stats())
        if self.l2:
            stats["l2_cache"] = vars(self.l2.stats())
        if self.coalescer:
