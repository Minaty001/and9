...
class WorkflowStep:
    ...
    def __init__(self, name: str, func: Callable, retry_policy: str = "skip", max_retries: int = 0):
        self.name = name
        self.func = func
...
class RecoveryWorkflow:
    ...
    def __init__(self, name: str = "default", rollback_manager: Optional[RollbackManager] = None):
        self.name = name
    ...
    def add_step(self, name: str, func: Callable, retry_policy: str = "skip", max_retries: int = 0) -> None:
        step = WorkflowStep(name=name, func=func, retry_policy=retry_policy, max_retries=max_retries)
            ...
            try:
                ...
                if step.retry_policy == RetryPolicy.RETRY:
                    if step.attempts <= step.max_retries:
                        logger.debug("Workflow %s: retrying step %s (attempt %d/%d)", self.name, step.name, step.attempts, step.max_retries)
                        self._steps.insert(i + 1, step)
                        continue
                    else:
                        logger.warning("Workflow %s: step %s exhausted retries", self.name, step.name)
                        success = False
                        break
                    ...
                    logger.warning("Workflow %s: rolling back due to step %s failure", self.name, step.name)
                    self.rollback()
                    success = False
                    break
                elif step.retry_policy == RetryPolicy.STOP:
                    logger.warning("Workflow %s: stopping due to step %s failure", self.name, step.name)
                    success = False
                    break

        self._total_duration_ms = (time.perf_counter() - self._start_time) * 1000
        logger.info("Workflow %s: finished (success=%s, duration=%.2fms)", self.name, success, self._total_duration_ms)
    ...
    def get_progress(self) -> dict:
        return {
            ...
            "steps": [{"name": s.name, "state": s.state.value, "duration_ms": round(s.duration_ms, 2),
                       "error": s.error, "attempts": s.attempts} for s in self._steps],
