...
{trend_html}
    ...
    def _render_overview(self, report: AnalyticsReport) -> str:
        ...
        cards = f"""
<div class="grid">
  <div class="card"><h3>Total Events</h3><div class="value">{total_events}</div><div class="sub">in this period</div></div>
  <div class="card"><h3>Active Users</h3><div class="value">{unique_users}</div><div class="sub">unique users</div></div>
  <div class="card"><h3>Operations</h3><div class="value">{total_ops}</div><div class="sub">total operations</div></div>
  <div class="card"><h3>Success Rate</h3><div class="value {success_class}">{success_rate}%</div><div class="sub">overall</div></div>
</div>"""

        insights_html = ""
        if report.insights:
            items = "".join(f'<div class="insight-item">{insight}</div>' for insight in report.insights)
            insights_html = f'<h3 style="margin-bottom:8px;font-size:15px;">Insights</h3>{items}'

        return f'<div class="section"><h2>Overview</h2>{cards}{insights_html}</div>'
...
<h2>Performance</h2>
<div class="grid">
  <div class="card"><h3>Avg Latency</h3><div class="value">{avg_lat}ms</div></div>
  <div class="card"><h3>P50 / P95 / P99</h3><div class="value" style="font-size:20px;">{p50} / {p95} / {p99} ms</div></div>
  <div class="card"><h3>Error Rate</h3><div class="value {error_class}">{error_rate}%</div><div class="sub">success: {success_rate}%</div></div>
  <div class="card"><h3>Slowest Endpoint</h3><div class="value" style="font-size:18px;">{slowest}</div></div>
  <div class="card"><h3>Busiest Hour</h3><div class="value">{busiest}</div></div>
</div>
</div>"""
    ...
    def _render_top_events(self, report: AnalyticsReport) -> str:
        ...
        return f'<div class="section"><h2>Top Events</h2><table><thead><tr><th>#</th><th>Event Type</th><th>Count</th></tr></thead><tbody>{rows}</tbody></table></div>'
    ...
    def _render_daily_trend(self, report: AnalyticsReport) -> str:
        ...
        hourly_section = ""
        if hourly_items:
            hourly_section = f'<h3 style="margin-top:20px;margin-bottom:8px;font-size:15px;">Hourly Distribution</h3><ul class="trend-list">{hourly_items}</ul>'

        return f'<div class="section"><h2>Daily Trend</h2><ul class="trend-list">{items}</ul>{hourly_section}</div>'
