...
class RoleManager:
    ...
    def __init__(self, default_role: str = "user", max_roles_per_user: int = 5):
        self._default_role = default_role
        self._max_roles_per_user = max_roles_per_user
    ...
    def assign_role(self, user_id: str, role_name: str) -> bool:
        ...
        if role_name not in perms.roles:
            if len(perms.roles) >= self._max_roles_per_user:
                return False
    ...
    def get_user_roles(self, user_id: str) -> List[Role]:
        ...
        if user_id not in self._user_permissions:
            return [self._roles.get(self._default_role, self._roles.get("user"))]
        ...
        for name in role_names:
            ...
            if r:
                result.append(r)
        default = self._roles.get(self._default_role)
    ...
    def get_user_permissions(self, user_id: str) -> Optional[UserPermissions]:
        ...
        if user_id not in self._user_permissions:
            ...
            if self._default_role in self._roles:
                perms.roles.append(self._default_role)
