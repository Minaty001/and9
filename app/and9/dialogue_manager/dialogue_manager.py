...
from app.and9.dialogue_manager.state_manager import (
    DialogueStateTracker,
    TaskState,
    TaskStatus,
)
from app.and9.dialogue_manager.task_manager import TaskManager
from app.and9.dialogue_manager.working_memory import (
    WorkingMemory,
    ShortTermMemory,
    ActiveTaskMemory,
    DialogueConfig,
...
from app.and9.dialogue_manager.action_planner import ActionPlanner, ExecutionPlan
...
class DialogueManager:
    ...
    def process(self, user_message: str) -> dict[str, Any]:
        ...
        try:
            ...
            if ref_meta.get("resume_requested"):
                ...
                if resumed:
                    return self._continue_task(resumed, result, start)
                ...
                if (current_active and current_active.is_active
                    ...
                    return self._continue_task(current_active, result, start)

            # ── Step 4: Intent Detection (via AND9 router) ───────
            ...
            # ── Step 5: Get slot definitions for this intent ──────
            required_slots = get_required_slot_names(intent_name)
            optional_slots = get_optional_slot_names(intent_name)

            # ── Step 6: Check for active task ────────────────────
            active_task = self.state_tracker.get_active_task()

            # ── Step 7: Handle active task with chat/default intent ──
            # If the active task is waiting for info and the new message
            # doesn't match any specific intent (chat=default), assume
            # the user is continuing the active task (filling a slot).
            # BUT only if the message does NOT look like a new topic
            # question — "what's the weather?" is an interruption, not
            # a slot-filling answer.
            if (active_task and active_task.is_active
                ...
                # Treat as continuation of active task
                task = active_task
                logger.debug("Assuming continuation of task %s (msg='%s')",
                             task.task_id, resolved_message[:40])

                # Try to fill the waiting slot from this message
                self._fill_from_message(task, resolved_message, params)
                self._update_entity_memory(task, resolved_message)

                # Copy slots from a recently completed task if references resolved
                self._fill_from_completed_task(task, ref_meta)

                return self._handle_task_state(
                    task, user_message, result, start
                )

            # ── Step 8: Interruption Check ───────────────────────
            if active_task and active_task.is_active and active_task.status not in (
                ...
                if is_interruption:
                    # Pause current task, handle new intent
            ...
            if active_task and active_task.intent == intent_name and active_task.is_active:
                ...
                # Pause the previous active task if switching to a new intent
                if active_task and active_task.is_active:
            ...
            # ── Step 9: Fill slots from resolved message ─────────
            self._fill_from_message(task, resolved_message, params)
            self._update_entity_memory(task, resolved_message)
            ...
            # ── Step 10: Check readiness and respond ─────────────
            return self._handle_task_state(
                task, user_message, result, start
            )
    ...
    def _handle_task_state(self, task: TaskState, user_message: str,
        ...
        if self.slot_filler.all_required_filled(task):
            ...
            # If we just filled a slot, get the next question
            if task.waiting_for:
                question = self.slot_filler.get_next_question(task)
            else:
                # No waiting_for determined yet — get first missing slot question
                question = self.slot_filler.get_next_question(task)
    ...
    def _execute_and_respond(self, task: TaskState, plan: ExecutionPlan,
        ...
        if self._executor:
            try:
                ...
                # The executor expects (action_type, params) signature
                ...
                result["error"] = str(e)
                result["time_ms"] = (time.perf_counter() - start) * 1000
                self._update_memory("", result["response"], task.intent, task.task_id)
                return result
        else:
        ...
        self._update_memory("", response, task.intent, task.task_id)
        return result
    ...
    def _handle_interruption(self, user_message: str, resolved_message: str,
        ...
        if new_intent in ("flashlight", "wifi", "bluetooth", "volume", "camera", "home"):
            ...
            if self.slot_filler.all_required_filled(task):
                ...
                plan = self.action_planner.plan(task)
                if plan.can_execute:
                    exec_result = self._execute_and_respond(task, plan, result, start)
    ...
    def _handle_cancel(self, message: str, result: dict, start: float) -> dict:
        ...
        if cancel_target and active and active.intent == cancel_target:
        ...
        # Cancel the active task regardless
    ...
    def _continue_task(self, task: TaskState, result: dict,
        ...
        if self.slot_filler.all_required_filled(task):
            ...
            if plan.can_execute:
                ...
                result["response"] = f"Chaliye, {task.intent} jaari rakhte hain! 😊"
    ...
    def _is_new_topic_request(message: str) -> bool:
        ...
        if re.search(r'^(what|where|when|why|how|who|which)\b', msg):
            return True
        if msg.endswith('?'):
            return True
        # Topic shift markers
        if re.search(r'^(actually|by the way|btw|anyway|hey|listen|wait|oh)\b', msg):
            return True
        return False
