...
def execute(action_type: str, params: Dict[str, Any] = None,
    ...
    if not action_info:
        ...
        return {
            ...
            "action": "ERROR",
            "payload": {},
            "metadata": {"failure_reason": "unknown_action_type"},
        }
    ...
    if not handler_path:
        return {
            ...
            "payload": {},
            "metadata": {"failure_reason": "no_handler_defined"},
        }

    try:
        # Call the handler to build the action payload
        handler_result = _call_handler(handler_path, action_type, params, events_sys)
        payload = handler_result.get("payload", {})

        # —— Chrome Firewall post-check ————————————————————————
        # Verify the actual payload doesn't route to Chrome.
        # Even for youtube_search/youtube_play the handler may produce
        # a YouTube app intent (not Chrome), so we check the payload.
        try:
...
def _call_handler(handler_path: str, action_type: str,
    ...
    Returns:
        Handler result dict.
    """
    from app.android.skill_registry import execute_skill
