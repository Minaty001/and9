...
## 2024-03-25 - Button Focus Visibility
**Learning:** Native `<button>` elements across the admin panel lacked `:focus-visible` styles, making keyboard navigation difficult as users couldn't see which button was currently focused.
**Action:** Added a global `button:focus-visible` CSS rule to ensure all buttons consistently display a clear outline when focused via keyboard, aligning with the existing input focus style (`#FFA500`).
